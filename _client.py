...
class ZaloAPI(object):
	...
	def onLoggingIn(self, phone=None):
		...
		"""
		logger.debug("API ZLAPI".format(phone))
	...
	def onLoggedIn(self, phone=None):
		...
		Args:
			phone: The phone number of the client
		"""
		logger.login("ZLAPI ĐÃ ĐĂNG NHẬP VÀO SỐ {} NÀY🦍.".format(phone))
...
def sendTodo(self, target_id, content, mid, author_id, thread_type, thread_id=None):
	    ...
	    Args:
	        target_id (int | str): User ID to send todo to
	    ...
	    Raises:
	        ZaloAPIException: If request failed
	        ZaloUserError: If thread type is invalid
	    """
	    params = {
	        "zpw_ver": 645,
	        "zpw_type": 30
	    ...
	    if thread_type == ThreadType.USER:
	        ...
	        if not thread_id:
	            raise ZaloUserError("Group ID is required for group todos")
	        to_id = thread_id
	        is_group = True
	        src = 5
	    else:
	    ...
	    url = "https://board-wpa.chat.zalo.me/api/board/personal/todo/create"
	    payload["params"] = self._encode(payload["params"])
